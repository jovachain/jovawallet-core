...
fn sol_sign_message_matches_solders() {
    ...
    let raw =
        std::fs::read_to_string("../../tools/sol-vector-capture/captures/sign_message.signed.json")
    ...
    assert_eq!(
        sig, expected_sig,
        "raw-message signature mismatch vs PyNaCl reference"
    );
}
